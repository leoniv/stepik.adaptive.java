...
public class ThreadSafe5Quiz {
    ...
    private static Future< ? > run(
            final Consumer< Integer > operation, final int amount
            ,final CyclicBarrier barrier, final CountDownLatch done) {
    ...
    public static class ThreadSafe5 {
        ...
        public void withdraw(final int amount) {
            w.lock();
            try {
                System.err.printf("Withdraw %d%n", amount);
                if (balance >= amount) {
                    balance -= amount;
                }
            } finally {
        ...
        public void deposit(final int amount) {
             w.lock() ;
            try {
                System.err.printf("Deposit %d%n", amount);
                balance += amount;
            } finally {
               w.unlock();
            }
